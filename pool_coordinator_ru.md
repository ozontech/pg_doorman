...
2. **Изъятие**: у `analytics` spare = 40 − 3 = 37 выше минимума. Закрывается самое старое idle-соединение (возраст > 5с).
...
1. Eviction attempt: `app_service` spare=0, skip. `analytics` spare=7, но idle connections моложе 5с — skip.
...
| Пользователь | starving | queued_clients | Score |
|-------------|-----------------|---------------|-------|
...
Изъятие: у `app_service` spare=40, но нет idle-соединений. Таймаут.
...
**analytics побеждает** — `starving=1` (user ниже guaranteed minimum) даёт абсолютный приоритет над user'ами, у которых minimum уже есть.
...
│  │  - evict у user с макс. spare                      │    │
│  │    (spare = connections - min_pool_size)            │    │
...
│     │     │  для каждого чужого пользователя (по spare DESC):
│     │     │    пропуск если spare ≤ 0 (на/ниже минимума)
...
| `src/pool/mod.rs` | Создание координаторов в `from_config()`, `spare_above_min()` на ConnectionPool |
