...
fn download_openssl_source(target_dir: &Path) {
    ...
    fs::rename(&extracted, target_dir).unwrap_or_else(|e| {
        panic!(
            "rename {} -> {}: {e}",
            extracted.display(),
            target_dir.display()
...
impl Build {
    ...
    pub fn try_build(&mut self) -> Result<Artifacts, String> {
        ...
        // Apply pg_doorman patches to OpenSSL source before configure
        apply_patches(&inner_dir)?;
...
/// Apply .patch files from the patches/ directory to the OpenSSL source tree.
/// Patches are applied in sorted order (001-xxx.patch, 002-yyy.patch, ...).
fn apply_patches(openssl_src_dir: &Path) -> Result<(), String> {
    ...
    for patch_path in &patches {
        ...
        println!(
            "cargo:warning=openssl-src: applying patch {}",
            patch_name
        );
        ...
        let status = Command::new("patch")
            ...
            .status()
            .map_err(|e| format!("failed to run patch for {patch_name}: {e}"))?;
    ...
    if !patches.is_empty() {
        println!(
            "cargo:warning=openssl-src: {} patch(es) applied",
            patches.len()
        );
    }
