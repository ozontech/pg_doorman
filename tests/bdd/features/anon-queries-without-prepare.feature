...
Feature: .NET anonymous queries without prepare on server side
  ...
  Scenario: Run .NET anonymous queries without prepare on server side
    When I run shell command:
      ...
      tests/dotnet/run_test.sh anon_queries anon_queries_without_prepare_on_server_side.cs
      """
    Then the command should succeed
    And the command output should contain "anon_queries_without_prepare_on_server_side complete"
