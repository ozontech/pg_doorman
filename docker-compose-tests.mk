...
docker-compose-test-dotnet: docker-compose-up
	docker compose exec -T dotnet_test /bin/bash -ec 'mkdir -p /tests/prj && cd /tests/prj && \
      ...
      cd .. && rm -rf ./anon_queries && mkdir -p ./anon_queries && cd ./anon_queries && dotnet new sln --name AnonQueries && dotnet new console --output . && dotnet add package Npgsql && cp -av ../../data/anon_queries_without_prepare_on_server_side.cs ./Program.cs && dotnet run Program.cs'
