...
- **Operator console with SSO** — single-page diagnostic UI on the same listener as `/metrics`. Authenticated by Basic auth, or by a JWT from an external SSO proxy (oauth2-proxy / Keycloak). Three roles: anonymous read, SSO read-only (with SQL text and logs), Admin (Basic) for management. [Learn more](https://ozontech.github.io/pg_doorman/guides/web-ui.html).
