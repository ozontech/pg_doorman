...
## 16. Backend extensions для Grafana parity (nice-to-have)

Этот раздел изначально утверждал, что UI требует двух must-have backend-добавлений (per-pool error counter, wait time percentiles). При реализации phase 3a выяснилось, что **оба значения уже доступны** в `PoolStats`:

- `PoolStats.errors: u64` — aggregated per-pool error counter (используется в `/api/pools[i].errors_total` и в `/api/overview.errors_count_total`).
- `PoolStats.wait_percentile.{p50, p90, p95, p99}: u64` — wait time percentiles (используется в `/api/pools[i].wait_p95_ms`).

UI получает их без дополнительной работы. Все элементы ниже — **nice-to-have для Grafana parity и расширений**, не блокируют MVP.

### 16.1 Per-pool error counter с разбивкой по kind (Grafana drill-down)

`/api/pools[i].errors_total` сегодня — aggregated число. Для Grafana дашбордов полезно иметь разбивку по типу ошибки (`auth`, `query`, `tls`, `disconnect`, `timeout`).

**Добавить (nice-to-have):** `pg_doorman_pool_errors_total{user, database, kind}` Prometheus-counter. Инкрементируется в существующих error-сайтах через тонкий helper:
...
Используется в Grafana для разбора причины: `pg_doorman_pool_errors_total{kind="tls"}` отдельно от `kind="auth"`.

UI продолжает использовать aggregated `PoolStats.errors`.

### 16.2 Wait time percentiles в Prometheus exporter

`/api/pools[i].wait_p95_ms` уже работает на JSON-стороне. Для Grafana добавить тот же показатель как `pg_doorman_pools_wait_time_percentile{user, database, percentile}` gauge через HDR histogram (тот же механизм, что у query/xact percentiles).

UI это не нужно — оно потребляет JSON.

### 16.3 Прочие nice-to-have для 3.8.1

- `pg_doorman_pool_paused{user, database}` (0 | 1) — Grafana parity (`/api/pools[i].paused` уже отдаёт это поле для UI).
