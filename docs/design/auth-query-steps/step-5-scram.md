...
## 5.4 Return type change

`try_auth_query()` needs to return `ClientKey` alongside `ConnectionPool` for
passthrough mode. Update return type:

```rust
struct AuthQueryResult {
    pool: ConnectionPool,
    client_key: Option<Vec<u8>>,  // Some for SCRAM, None for MD5
}
```

Store in client session for use when backend connection is established (Step 6).
