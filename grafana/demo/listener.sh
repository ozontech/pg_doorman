...
# `pg_doorman_pools_clients{user="session_user"}` non-zero on idle for the
# Web UI and Grafana to surface.
set -e

until pg_isready -h pg_doorman -p 6432 -U session_user -d app_db; do
...
for i in 1 2 3; do
    PGPASSWORD=session_pass psql \
        -h pg_doorman -p 6432 -U session_user -d app_db \
        -c "LISTEN app_events;" \
