...
# the session_user pool. Lower concurrency than the transaction-mode pgbench
...
until pg_isready -h pg_doorman -p 6432 -U session_user -d app_db 2>/dev/null; do
    sleep 1
done
...
exec pgbench \
    -h pg_doorman -p 6432 -U session_user \
