...
Давление на пул (pool pressure) описывает, как pg_doorman ведёт себя,
когда множество клиентов одновременно запрашивают серверное соединение,
а в idle-пуле пусто. Решение о том, кто получит соединение, кто будет
ждать, кто инициирует свежий backend-connect, а кому будет отказано,
принимают два механизма: **anticipation + bounded burst** внутри каждого
пула `(database, user)` и общий для всех пулов **coordinator**,
ограничивающий суммарное число backend-соединений к базе данных.
...
best-effort. Жёсткий пол описан в разделе troubleshooting.
...
пулу нужно сделать только `connect()`. Это и есть **head-of-line
blocking внутри одного пула**.
