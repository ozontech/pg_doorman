...
принимают два механизма: пер-пуловые **anticipation + bounded burst**
внутри каждого пула `(database, user)` и кросс-пуловый **coordinator**,
ограничивающий общее число backend-соединений на базу данных.
...
Следствие: под нагрузкой `min_pool_size` соблюдается по best-effort.
Жёсткий пол описан в разделе troubleshooting.
...
`max_db_connections`. Worst case даёт **120 одновременных backend-соединений**
...
`N × pool_size` из предыдущего раздела остаётся единственным ограничением. С
...
нужны больше всех, получали их первыми. Arbiter работает как одна tokio-задача,
раздающая reserve-permit'ы из приоритетной кучи.
...
По устройству координатор работает как *cap*, не *очередь*: он стоит вам
...
Два параметра anticipation/burst поддерживают только глобальную настройку;
пер-пуловые оверрайды для них недоступны.
...
- `pool_size / 4` для самого маленького пула, использующего этот
  параметр. Выше этого значения cap теряет смысл: половина пула может одновременно
  быть в полёте, что разрушает сглаживание.
- `(PostgreSQL max_connections - superuser_reserved_connections) / (10 × N pools)`,
  где `N pools` обозначает все пулы, делящие этот инстанс PostgreSQL. Выше этого значения
...
коннектов равен `N pools × scaling_max_parallel_creates`. Если у вас один
...
Резерв работает как *временный клапан переполнения*, не как дополнительная
...
частота одновременных коннектов равна `pools × scaling_max_parallel_creates`,
...
эвристика не применяется: `min_pool_size = pool_size` это легитимная
