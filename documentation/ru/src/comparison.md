...
| `SHOW INTERNER` (записи / байты / preview по половинам) | Да | Нет | Нет |
| Ограниченный prepared-кеш (TTL у анонимных, per-client LRU split) | Да | Нет (только named, неограниченный `max_prepared_statements`) | Нет |
