...
читает эту колонку как `text` или `json`/`jsonb`: содержимое должно
быть JSON-объектом с параметрами запуска PostgreSQL для конкретного
пользователя. Dedicated-режим игнорирует её и пишет предупреждение.
