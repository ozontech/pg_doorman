...
минимальный `SilentCallback` без обычных polling-эффектов. Он
...
- `debug` — любые остальные успешные чтения (`2xx`), анонимные или
  ...
  логируются на `debug`, а `RUST_LOG=info` остаётся для admin-действий,
  auth-трафика и ошибок.
...
в 1,5 секунды. В заголовке — состояние (OK / DEGRADED / CRITICAL) и
кнопка **Экран инцидента**.

Плитки страницы:

- **Баннер Patroni-fallback** — появляется в верхней части страницы,
  когда хотя бы один пул сообщает `fallback_active = true`.
  Перечисляет все пулы, которые в данный момент идут на резервный
...
- **Sparkline по золотым сигналам** — query p95, qps, ошибок/с,
  насыщение. У каждой плитки есть структурированная подсказка:
  определение, источник (соответствующая команда `SHOW`), формула,
  пороги и ссылка в документацию.
...
- **Топ SQLSTATE** — агрегированная частота кодов ошибок по всем
  ...
  краткое описание, счётчик. Детализация по конкретному пулу находится
  в **Деталях пула**.
- **Ресурсы (свёрнуто по умолчанию)** — RSS процесса, CPU,
  файловые дескрипторы, баланс tokio-потоков, сокеты, query
  interner. Секция свёрнута по умолчанию: это диагностические
  подробности, а не сигнал тревоги.

Admin-действия находятся на страницах, к которым относится их область
действия. См. [Admin-действия](#admin-действия).
...
доля ошибок, saturation, флаг fallback и мини-график в каждой
...
- **Pool scaling** — создания бэкендов, ожидания на гейте,
  исчерпание бюджета, anticipation hits, fallback-creates. См.
  [Пул под нагрузкой (продвинутое)](../tutorials/pool-pressure.md).
- **Сработавшие пороги** — проверки здоровья, которые сейчас активны
  для этого пула.
...
Таблица всех клиентов с пагинацией и периодическим обновлением.
Фильтры лежат в URL, поэтому текущий вид можно отправить в канал
инцидента:
...
`SHOW CLIENTS`. Вместе с **Серверами** эта страница показывает связку
...
Таблица всех бэкенд-соединений, которые pg_doorman держит в данный
момент. Есть пагинация и периодическое обновление. Источник —
`SHOW SERVERS`.
...
Вместе с **Клиентами** используется для сопоставления зависшего
запроса: возьмите `server_id` из строки клиента, откройте эту
страницу и найдите pid в `pg_stat_activity`.
...
закрытая вкладка не держит ring buffer активным.

Фильтры лежат в URL: `level`, `q` (подстрока), `paused`, `scroll`.
URL с фильтрами сохраняется после обновления страницы и подходит для
шеринга:
...
число drops с момента открытия. Путь с персональными данными:
доступен только `Sso` / `Admin`.

При `[web].log_tap_max_entries = 0` страница показывает панель с
инструкциями вместо пустого потока: стриминг логов выключен в
текущей конфигурации, пока значение не увеличат и pg_doorman не
перезапустят.

### Конфиг и состояние (`/config`)

Зеркало `SHOW CONFIG`, `SHOW DATABASES`, `SHOW USERS` только для чтения,
...
Панель «startup parameters» перечисляет все пары пул × переопределение
из каскада — полезно при проверке правки TOML до перезагрузки.
...
для роли admin). Она отправляет тот же `POST /api/admin/reload`, что и
psql admin console, через подтверждение по вводу слова `RELOAD`.
См. [Admin-действия](#admin-действия).

### Экран инцидента (`/wall`)

Экран для тех же данных Обзора, рассчитанный на настенный дисплей в
дежурной зоне. Сайдбар и подсказки скрыты; шесть крупных KPI-плиток
(max p95, ошибок/с, max saturation, waiting, oldest active, число
пулов) стоят под полноразмерной картой насыщения пулов. Когда хотя бы
один сигнал превышает критический порог, страница показывает
пульсирующую красную рамку. Внизу перечислены последние admin-события,
чтобы всплеск метрики можно было сопоставить с предшествующим
действием.

Страница берёт `navigator.wakeLock("screen")`, чтобы экран не уходил
в режим сна, и выходит на `/overview` по **Esc**. Открывается из
заголовка **Обзора** («Open war room»), потому что это те же данные в
формате для большого экрана.
...
размер пула уменьшается по мере естественного освобождения соединений.
Подтверждение
...
После успешного действия в правом верхнем углу появляется уведомление:
«Config reload requested» или «PAUSE applied to `<db>`». При ошибке
показывается уведомление с телом ответа. Admin-
действия дополнительно пишутся в access-лог на уровне `info`
(`auth_role=admin auth_source=basic|sso`) и попадают в буфер admin-
событий. На графиках **Обзора** в момент действия появляется
вертикальная аннотация, а **Экран инцидента** показывает действие в
разделе «Recent admin events».
...
| Сочетание                            | Действие                                                                                          |
|--------------------------------------|----------------------------------------------------------------------------------------------------|
| <kbd>⌘ K</kbd> / <kbd>Ctrl K</kbd>   | Открыть command palette: перейти на страницу, найти пул по id, базе или пользователю и нажать **Enter**. |
| <kbd>?</kbd>                         | Открыть модальное окно со списком горячих клавиш.                                                  |
| <kbd>Esc</kbd>                       | Закрыть подсказку или модальное окно. На `/wall` возвращает на **Обзор**.                           |

Command palette при каждом открытии опрашивает `/api/pools`, поэтому
список пулов актуален, а не взят из момента загрузки страницы.
...
Палитра: Geist Sans для текста интерфейса, JetBrains Mono для числовых
колонок и идентификаторов; акцент — `#2563eb` для светлой темы, `#60a5fa`
для тёмной.

## Встроенная справка

Рядом с заголовком каждой плитки и секции стоит значок (i). Клик или
наведение открывает подсказку:
...
Структура подсказки одинаковая на всех страницах: поля на **Обзоре**,
в **Деталях пула** и в **Кешах** означают одно и то же.

## Toast-уведомления

Sonner Toaster показывает уведомления в правом верхнем углу. Его
используют admin-действия и нефатальные ошибки; остальная страница
продолжает работать, пока уведомление видно. Каждое уведомление
показывается 4 секунды. Toaster следует за активной темой.
