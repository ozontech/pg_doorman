...
В pg_doorman встроена одностраничная операторская консоль. HTTP-сервер тот же,
что отдаёт Prometheus-метрики; собранный фронтенд лежит внутри бинарника.
Запуск консоли не добавляет внешних зависимостей: один процесс, один бинарь,
один TCP-порт.

## Включение

Консоль настраивается в секции `[web]`. Старое имя секции `[prometheus]` тоже
...
литералу `"admin"`, консоль на старте молча переходит в режим «только
metrics». HTTP-сервер продолжает отдавать `/metrics`, но admin-эндпоинты иначе
оказались бы открыты любому. Задайте настоящий пароль до того, как включать
`ui = true`. Срабатывание этой проверки видно в логе по строке
...
|---|---|---|
| `/`, `/pools`, любой не-API путь | нет | Оболочка SPA. Отдаётся анонимно даже при `ui_anonymous = false`, чтобы прямой переход на deep link не открывал системный диалог Basic-авторизации браузера до того, как успеет отрендериться форма входа React. |
...
| `GET /api/version`, `/api/overview`, `/api/pools`, `/api/clients`, `/api/servers`, `/api/connections`, `/api/stats`, `/api/databases`, `/api/users`, `/api/auth_query`, `/api/config`, `/api/log_level`, `/api/pool_coordinator`, `/api/pool_scaling`, `/api/sockets`, `/api/prepared`, `/api/interner`, `/api/top/clients`, `/api/top/prepared`, `/api/apps`, `/api/events` | `Anonymous`, когда `ui_anonymous = true`, иначе `Sso` | Read-only JSON, повторяет формат `SHOW <admin-команда>`. |
| `GET /api/logs`, `/api/prepared/text/{hash}`, `/api/interner/top`, `/api/top/queries` | `Sso` | Read-only эндпоинты с персональными данными. `/api/logs` подключает буфер логов на первом запросе и отключает его через 2 минуты простоя. `/api/top/queries` возвращает первые ~120 символов SQL-текста из кэша. Эти данные не вынесены в публичную поверхность, потому что превью могут содержать литералы и идентификаторы клиентов. |
| `POST /api/admin/{reload,pause,resume,reconnect}` | `Admin` | Управляющие admin-операции. Семантика та же, что и у psql admin-протокола. |

## Роли доступа

Сервер на каждом запросе вычисляет одну из трёх ролей. Проверка роли работает
на стороне сервера; SPA дублирует её на клиенте только для того, чтобы скрыть
кнопки, которыми оператор всё равно не может воспользоваться.

| Роль | Как запрос её получает | Что роль даёт |
|---|---|---|
| `Anonymous` | Реквизитов нет, `[web].ui_anonymous = true`. | Read-only публичные `/api/*` из таблицы выше плюс `/metrics`. На пути с персональными данными и `/api/admin/*` возвращается `401`. |
| `Sso` | Валидный JWT в `Authorization: Bearer`, в cookie `sso_access_token=` или в query `?token=`, который **не** попадает в admin-группу. | Все read-эндпоинты, включая пути с персональными данными. На `POST /api/admin/*` отдаётся `403`. |
...
Когда в одном запросе есть и Basic, и SSO-токен, листенер предпочитает
Basic. Корректный admin-пароль даёт `Admin` независимо от состояния SSO.
...
Сравнение Basic-пароля идёт за постоянное время относительно настроенных
реквизитов. JWT валидируются по публичному ключу из
`[web].sso_public_key_file`; распарсенный ключ кэшируется на время жизни
процесса и перечитывается на `RELOAD`.

`fetch`-обёртка SPA шлёт `Accept: application/json`, и листенер на ней
...
`401 Unauthorized` отдаётся, когда реквизитов не пришло или ни один
вариант не прошёл парсинг и валидацию. `403 Forbidden` отдаётся, когда
реквизиты валидны, но роли не хватает для пути; тело —
...
SSO опциональный. По умолчанию (`[web].sso_enabled = false`) листенер
обслуживает только роли Anonymous и Admin (Basic). Чтобы подключить
внешний SSO-proxy:

1. Получите от SSO-провайдера RSA public key, которым он подписывает JWT,
   и сохраните его в PEM-файле (например, `/etc/pg_doorman/sso-public.pem`).
   Для oauth2-proxy ключ извлекается из приватного:
   `openssl rsa -in private.pem -pubout -out public.pem`. Для Keycloak ключ
   копируется из админ-консоли: Realm Settings → Keys.
...
| `sso_admin_groups` | Группы, которые поднимают SSO-пользователя до `Admin`. Пустой список оставляет каждый SSO-логин на read-only роли `Sso`. | `[]` |
| `trusted_proxies` | CIDR доверенных reverse-proxy. Пустой список — доверять только своему peer'у. См. [Журнал доступа](#журнал-доступа). | `[]` |

### Поднятие SSO-пользователей до Admin через group claim

По умолчанию SSO-логин получает роль `Sso` — read-only с доступом к логам и
SQL-текстам, но без `POST /api/admin/*`. Чтобы операторы могли запускать
управляющие admin-операции через SSO без раздачи Basic-пароля, настройте
`sso_groups_claim` и `sso_admin_groups`:
...
запрос разрешается в `Admin`. В access-логе это выглядит как
`auth_role=admin auth_source=sso`, поэтому SSO-админы по-прежнему
отличимы от Basic-админов. `/api/auth/config` отдаёт
...
пустой audience, нечитаемый PEM) листенер пишет причину в лог на уровне
...
заметил поломку, а не молчаливый фоллбэк:
...
OAuth/OIDC-флоу и возвращает браузер обратно с `?token=<jwt>`. SPA
сохраняет токен в `localStorage`, чистит URL от параметра и шлёт
`Authorization: Bearer <jwt>` на каждом следующем запросе.

В нижней части sidebar отображается имя текущего пользователя: `admin`
...
Поллер silent-refresh тикает раз в 60 секунд. Когда до `exp` остаётся
меньше 90 секунд, SPA открывает скрытый iframe с URL
`${origin}/?sso_silent=1`. Внутри iframe App-роутер рендерит
минимальный `SilentCallback` (без обычных эффектов поллинга), который
через `window.postMessage` отдаёт новый токен parent-окну. Если silent
refresh не удался:

- при наличии Basic-реквизитов SPA удаляет SSO-токен без редиректа и
...
Basic-пароль по умолчанию живёт только в памяти React и пропадает на
hard reload. Галочка **Remember me on this device** в форме входа
сохраняет его в `localStorage`, и консоль переживает перезагрузку.
Очистка site storage в браузере удаляет и Basic, и SSO-запись.

## Журнал доступа

После каждого ответа (200/401/403/404/5xx, включая скрейпы `/metrics`)
консоль пишет одну logfmt-строку на target `pg_doorman::web::access`:
...
- `peer` — адрес инициатора запроса. По умолчанию это TCP-peer. Когда
  TCP-peer попадает в `[web].trusted_proxies`, листенер парсит
  ...
  TCP-peer не в списке доверенных, заголовки proxy игнорируются.
- `auth_role` — `admin`, `sso`, `anonymous` или `rejected`.
- `auth_source` — `basic`, `sso` или `-`.
- `auth_user` — разрешённое имя пользователя или `-` для анонимов и
...
- `info` — все admin-действия (`POST /api/admin/*`), все чтения
  ...
  любые запросы с разрешённой ролью (Sso или Admin).
- `debug` — анонимные успешные чтения публичных API и скрейпы
  `/metrics`. Prometheus стучится каждые несколько секунд, SPA
  поллит overview/pools, поэтому держать эти запросы вне `info`
  важно: иначе `RUST_LOG=info` тонет в шуме скрейпов.

Отдельный target `pg_doorman::web::access` позволяет фильтровать
поток access-лога независимо от остальных логов. В выпадающем
фильтре на странице **Logs** этот target включается или
исключается одним кликом.

### Реальный IP клиента за reverse proxy

По умолчанию `peer` фиксирует TCP-адрес, который соединился с
листенером. За reverse proxy это адрес самого proxy. Чтобы видеть
реальный IP клиента, добавьте CIDR proxy в `[web].trusted_proxies`:
...
| `pg_doorman_web_auth_attempts_total` | counter | `role`, `source` | Попытки авторизации в разрезе разрешённой роли (`admin`/`sso`/`anonymous`/`rejected`) и источника (`basic`/`sso`/`none`). |
| `pg_doorman_web_requests_total` | counter | `status_class`, `role` | Запросы к Web UI в разрезе HTTP-статуса (`1xx`–`5xx`) и роли. |
...
Устойчивый рост `signature` означает, что SSO-proxy ротировал ключ, а
...
обычно указывает на сломанный proxy перед pg_doorman.
...
**Silent refresh не срабатывает.** SSO-proxy должен возвращать
свежий токен без полного экрана логина, когда iframe приходит с
активной сессией. У oauth2-proxy это включается флагом
`--silent-refresh=true`.
...
- **Overview** — индикатор health, четыре sparkline по golden signals
  (latency p95, traffic, errors/s, saturation), stacked area по
  соединениям, heatmap заполнения пулов, двойная ось wait +
  oldest-active-age, топ-5 ошибок по пулам и свёрнутая панель Resource
  detail.
- **Pools** — таблица с сортировкой и mini-sparkline в строках.
- **Pool detail** (`/pools/:poolId`) — детальный разбор: разбивка по
  SQLSTATE, oldest-active-age, кнопки pause / resume / reconnect.
- **Clients** — таблица из `/api/clients` с пагинацией, серверной
  фильтрацией и сортировкой.
- **Apps** — строка на каждый `application_name` с долей ошибок на
  1k запросов.
- **Caches** — таблица Prepared Statements с hit rate и карточка
  query interner (named / anonymous bytes).
- **Logs** — live-tail LogTap с фильтром по level / target и кнопками
  pause / auto-scroll.
- **Config & state** — свёрнутые панели: `[general]`, активный фильтр
  логов, кэш auth_query, databases, users, sockets, pool scaling,
  pool coordinator.
- **War room** (`/wall`) — шесть крупных плиток для incident bridge
  или стенда на стене.

## Сборка из исходников

Собранный фронтенд лежит в git по пути `frontend/dist/`, чтобы
RPM-, DEB- и Docker-пайплайны не зависели от node toolchain.
...
скрейпа, либо ставьте перед HTTP-сервером proxy, который добавляет
