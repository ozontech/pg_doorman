...
metrics». HTTP-сервер продолжает отдавать `/metrics`, но admin-эндпоинты
...
скрыть кнопки, которыми оператор всё равно не может воспользоваться.

| Роль | Как запрос её получает | Что роль даёт |
|---|---|---|
| `Anonymous` | Учётных данных нет, `[web].ui_anonymous = true`. | Read-only публичные `/api/*` из таблицы выше плюс `/metrics`. На пути с персональными данными и `/api/admin/*` возвращается `401`. |
...
По умолчанию SSO-логин получает роль `Sso` — read-only с доступом к логам
и SQL-текстам, но без `POST /api/admin/*`. Чтобы операторы могли
запускать управляющие admin-операции через SSO без раздачи Basic-пароля,
...
Silent-refresh запускается раз в 60 секунд. Когда до `exp` остаётся
...
- `peer` — адрес инициатора запроса. По умолчанию это TCP-peer. Если
  TCP-peer попадает в `[web].trusted_proxies`, сервер разбирает
...
access-лога независимо от остальных логов. В выпадающем фильтре на
...
сервером. За reverse proxy это адрес самого proxy. Чтобы видеть
...
**Silent-refresh не срабатывает.** SSO-proxy должен возвращать свежий
