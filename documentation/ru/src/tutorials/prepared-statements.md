...
- `prepared_statements_cache_size` (по умолчанию `8192`) задаёт
  ...
  LRU: выселение проходит за O(N) по всему кешу и не отправляет
...
- `client_anonymous_prepared_cache_size` (по умолчанию наследует
  ...
  пулового размера потолок per-client кеша.
...
Глобальный interner, который дедуплицирует тексты Parse, разделён
на две независимые хеш-таблицы.

- **NAMED** — тексты named prepared statements. Запись жива, пока
  pool- или client-кэш держит ссылку через `Arc<str>`. GC-таска
  убирает запись, когда её перестали удерживать снаружи interner'а;
  двухтактовый grace-период защищает запись, на которую ссылку
  взяли между двумя циклами sweep'а.
- **ANON** — тексты анонимных prepared. Запись истекает по
  ...
  до 3.7 — оставлено для legacy-инсталляций.

Когда interner и pool/client-кэши одновременно перестали держать
текст анонимного prepared, pg_doorman на следующий `Bind` отвечает
`ERROR: unnamed prepared statement does not exist` (SQLSTATE
`26000`). Это та же ошибка, что нативный PostgreSQL отдаёт в
аналогичной ситуации; стандартные драйверы реагируют повторным
`Parse`.

Бинарный апгрейд (`SIGUSR2`) переносит и NAMED, и ANON в новый
процесс. Анонимные записи попадают в новый ANON interner со свежим
`last_used`, и TTL отсчитывается заново с момента апгрейда.
...
`kind`, `bytes`, `idle_ms` (`-1` для named — там нет last-used,
вместо него отслеживается состояние GC) и 120 первых символов
текста запроса.

`RESET INTERNER` чистит обе половины. Активные клиенты переparsят
при следующем использовании. Команда диагностическая.
