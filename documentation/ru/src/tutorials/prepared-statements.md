...
`DOORMAN_<N>` на бекенде. План попадает в реестр именованных statement бекенда и
...
- `prepared_statements_cache_size` (по умолчанию `8192`) задаёт
  ...
  `Close` на бекенд (другие клиенты могут ещё держать `Arc`).
...
(SQLSTATE `26000`). Это та же ошибка, что нативный PostgreSQL
отдаёт в аналогичной ситуации; стандартные драйверы реагируют
повторным `Parse`.
