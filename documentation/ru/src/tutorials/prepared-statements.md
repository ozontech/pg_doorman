...
```text
  ...
  Анонимный (name="")     До следующего           Нет именованной
                          анонимного Parse        записи для повторного
                          или конца сессии        использования
...
| ---------------------------------------- | :----------: | ----------------------------------------------------------------------- |
| `prepared_statements`                    | `true`       | Включает remap и кеширование prepared statements. `false` отключает функцию. |
| `prepared_statements_cache_size`         | 8192         | Размер кеша пула в записях. Должен быть больше 0, пока `prepared_statements = true`. |
| `server_prepared_statements_cache_size`  | наследует `prepared_statements_cache_size` | Размер LRU на серверное соединение для имён `DOORMAN_<N>`. `0` отключает хранение на бэкенде, но не remap на уровне пула. |
...
- **Server-level LRU.** На каждом backend ведётся свой
  ...
  задан). При достижении лимита backend отправляет `Close` на наименее
...
- Снизьте `server_prepared_statements_cache_size`, чтобы server-level LRU
...
server-level LRU работает на потолке, а ротация — второй способ
