...
`dnf install pg-doorman`, пакетный менеджер выполняет шаг 1 атомарно
(`dpkg` пишет во временный путь и переименовывает, `rpm` делает то
же). Идиоматичный путь — `apt-get install --only-upgrade pg-doorman`
или `dnf upgrade pg-doorman`. Ручной `install` ниже — для разворотов
прямо бинарём, без пакетного менеджера.
```
...
#    в юните `systemctl reload` запускает полный binary upgrade
#    (сначала валидация `pg_doorman -t`, потом миграция состояния,
#    потом drain старого процесса). systemd шлёт сигнал в MainPID,
#    поэтому выбирается ровно один процесс, даже если на хосте есть
#    другие инстансы pg_doorman. Прямой `kill -USR2 $(pgrep -f
#    /usr/bin/pg_doorman)` работает, но pgrep матчит по командной
...
# 4. Проверьте: systemd подхватил новый MainPID (Type=notify получает
#    `MAINPID=<new_pid>` от чайлда при handoff). Состояние active и
#    админ-консоль подтверждают, что клиенты на месте.
systemctl show -p MainPID --value pg_doorman.service
psql -h pgdoorman -p 6432 -c 'SHOW POOLS;'  # отвечает новый процесс
```

```admonish tip title="Stand-alone установки без systemd"
Если процесс не управляется systemd, читайте PID-файл, который пишет
daemon (`daemon_pid_file`, по умолчанию `/tmp/pg_doorman.pid`), вместо
парсинга `pgrep`: `kill -USR2 "$(cat /var/run/pg_doorman/pg_doorman.pid)"`.
Foreground без systemd — шлите сигнал в PID супервизора процесса.
```
...
[Service]
Type=notify
# `exec` достаточно: новый процесс при SIGUSR2 порождается через
# Command::spawn (fork + execve), он попадает в exec-scope notify.
# `all` тоже работает, но `exec` сужает поверхность только до того,
# что действительно делает pg_doorman.
NotifyAccess=exec
ExecStart=/usr/bin/pg_doorman /etc/pg_doorman.yaml
# `systemctl reload` запускает полный binary upgrade (валидация
# конфига, миграция состояния, drain старого процесса). SIGUSR2 в
# pg_doorman сначала прогоняет `-t` и отменяет reload при битом
# конфиге, поэтому такой сигнал безопасно вешать на reload.
ExecReload=/bin/kill -SIGUSR2 $MAINPID
# `systemctl stop` шлёт SIGTERM в main pid для graceful drain.
ExecStop=/bin/kill -SIGTERM $MAINPID
# Перезапуск всегда: возвращаем пул на ноге даже после ручной
# остановки. Соответствует ожиданию HA на стороне PostgreSQL, что пул
# никогда не лежит на хосте.
Restart=always
RestartSec=5s
# pg_doorman тяжёл по дескрипторам: каждый клиент и каждый backend —
# отдельный fd, плюс служебные pipe. 65536 покрывает большинство
...
User=postgres
Group=postgres
# Имя в journalctl: `pg-pooler` вместо имени бинаря; удобно для
# `journalctl -u pg_doorman -t pg-pooler`.
SyslogIdentifier=pg-pooler
TimeoutStopSec=30s
```

`systemctl reload pg_doorman` запускает binary upgrade: pg_doorman
прогоняет `-t` на новом бинаре, отменяет reload при битом конфиге,
иначе порождает новый процесс и дренирует старый. `UPGRADE;` из
админ-консоли идёт по тому же code path. Подбирайте `TimeoutStopSec`
с запасом над `shutdown_timeout` из `pg_doorman.yaml`, иначе systemd
прибьёт старый процесс SIGKILL-ом до завершения дренажа.

Прод-установки часто добавляют сверху ограничения ресурсов:
`MemoryMax=`, `Nice=-15`, `CPUAffinity=2,3,4,5,6,7,8,9`. Это
workload-специфика и она ортогональна контракту upgrade.


или `UPGRADE;` из админ-консоли. `TimeoutStopSec` подбирайте под
значение `shutdown_timeout` в `pg_doorman.yaml` плюс небольшой запас,
иначе systemd прибьёт процесс SIGKILL до окончания дренажа.
...
- [ ] В systemd unit указано `Type=notify` с `NotifyAccess=exec`,
      `ExecReload=/bin/kill -SIGUSR2 $MAINPID` (тогда `systemctl
      reload` запускает полный binary upgrade с валидацией конфига)
      и `TimeoutStopSec`, покрывающий `shutdown_timeout` с запасом
