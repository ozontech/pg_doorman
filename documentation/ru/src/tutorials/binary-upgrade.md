# Обновление бинаря с переносом сессий

pg_doorman умеет заменить исполняемый файл на работающем сервере по
`SIGUSR2` и перенести уже открытые клиентские сессии в новый процесс.
Свободный клиент остаётся на том же TCP-соединении: старый процесс
передаёт его файловый дескриптор через Unix-сокет вместе с ключом
отмены запроса, параметрами PostgreSQL-сессии и кешем prepared
statements.
