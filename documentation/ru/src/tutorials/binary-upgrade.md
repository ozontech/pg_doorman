...
же). Обычный путь — `apt-get install --only-upgrade pg-doorman`
или `dnf upgrade pg-doorman`. Ручной `install` ниже нужен только там,
где бинарник раскладывают без пакетного менеджера.
```

```bash
# 1. Положите новый бинарник во временный файл в том же каталоге и
#    переименуйте поверх целевого пути. Финальный `mv` атомарен внутри
#    одной файловой системы, поэтому работающий процесс остаётся на
#    старом inode. Не делайте `cp` поверх цели: он перезаписывает живой
#    inode и может уронить старый pg_doorman через SIGBUS / SIGSEGV.
...
его с `-t` и текущим конфигом. После атомарной замены из быстрого
старта этот путь уже указывает на новый бинарник, поэтому проверяется
именно тот процесс, который должен принять трафик. Если проверка
проваливается, обновление отменяется, а старый процесс продолжает
обслуживать трафик. В логах появляется баннер:
