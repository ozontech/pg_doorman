...
# 3. Запустите обновление. При systemd `Type=notify` команда шлёт
#    SIGUSR2 в MainPID, который отслеживает systemd, — выбирается
#    ровно один процесс, даже если на хосте есть другие инстансы
#    pg_doorman или зомби-чайлды. Прямой `kill -USR2 $(pgrep -f
#    /usr/bin/pg_doorman)` работает, но pgrep матчит по командной
#    строке и может попасть во все инстансы сразу — поэтому
#    пакетные установки используют systemctl.
...
[Service]
Type=notify
# NotifyAccess=all нужен, чтобы новый процесс, который порождает
# SIGUSR2, мог достучаться до notify-сокета systemd и передать
# MainPID. Без него systemd молча продолжает следить за старым,
# умирающим PID.
NotifyAccess=all
ExecStart=/usr/bin/pg_doorman /etc/pg_doorman.yaml
ExecReload=/bin/kill -SIGHUP $MAINPID
# Перезапускать после падения, но с паузой, чтобы плохой релиз не
# зациклился. on-failure не реагирует на штатный `systemctl stop`.
Restart=on-failure
RestartSec=5s
# pg_doorman тяжёл по дескрипторам: каждый клиент и каждый backend —
# отдельный fd, плюс служебные pipe. Дефолтный soft-лимит 1024 — это
# первая стена в продакшене.
LimitNOFILE=1048576
# Сервисная учётка, которой принадлежат /var/run/pg_doorman и PID-файл.
# Использовать root не нужно, если listener слушает порт выше 1024.
User=pg_doorman
Group=pg_doorman
# Доставка сигналов: SIGUSR2 при upgrade должен дойти и до родителя,
# и до чайлдов; финальный SIGTERM при stop — в лидера группы. mixed
# шлёт stop-сигнал главному pid, а cleanup — в cgroup.
KillMode=mixed
TimeoutStopSec=120
```

`ExecReload=SIGHUP` отвечает за reload конфига; обновление бинаря
запускается отдельно — `systemctl kill -s SIGUSR2 pg_doorman.service`
...
- [ ] В systemd unit указано `Type=notify` с `NotifyAccess=all`,
      `ExecReload=/bin/kill -SIGHUP $MAINPID` для reload конфига и
      `TimeoutStopSec`, покрывающий `shutdown_timeout` с запасом
