...
только к **новым** серверным подключениям. Бэкенды, которые уже выданы
клиентам, продолжают работать со значениями, сохранёнными при создании
пула. При следующем переподключении клиента pg_doorman снова выполнит
`auth_query`, прочитает обновлённую строку, пересоздаст динамический пул
и будет использовать новые значения при последующих запусках бэкендов.
