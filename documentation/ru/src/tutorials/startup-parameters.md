...
Эффективный каскад per pool виден через admin SQL-консоль:
...
Web UI рендерит тот же набор в секции «Startup parameters
...
Разумная отправная точка для алерта: ненулевая скорость роста
