...
| **SIGUSR2** | Горячая замена процесса. Валидирует новый бинарь и конфиг флагом `-t`, запускает новый процесс, передаёт ему слушающий сокет и переносит сессии, где это возможно. **Рекомендуется для обновлений.** См. [Горячая замена процесса с переносом сессий](binary-upgrade.md). |
