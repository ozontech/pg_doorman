...
pg_doorman экспортирует состояние давления на пул через три источника.
Admin-консоль и Prometheus содержат одни и те же счётчики, выбирайте то,
что подходит вашему стеку мониторинга. Периодическая строка статистики —
единственный источник, показывающий внутренний учёт пула; с неё и
начнём.

### Строка статистики

Группа `servers=` в строке статистики показывает собственное представление
пула о себе рядом с соединениями, известными реестру статистики:
...
| Поле | Значение |
|---|---|
| `servers` / `active` / `idle` | Серверные соединения, зарегистрированные в реестре статистики, и их состояние. |
| `tracked` | Внутренний счётчик соединений пула (`slots.size`) — сколько соединений пул считает своими. |
| `waiters` | Длина очереди прямой передачи пула. |

**В установившемся режиме `tracked` и `servers` совпадают.** Они ведутся
разными путями, поэтому отдельная выборка может отличаться на соединение,
пойманное в момент создания или закрытия. Расхождение, которое держится
несколько выборок подряд, — это разъезд счётчика, и его стоит разбирать:
`slots.size` управляет фоновым пополнением (`slots.size >= max_size`
пропускает цикл) и порогом прогрева упреждающего ожидания
(`scaling_warm_pool_ratio` от `pool_size`). Завышенный выше порога, он
заставляет пул и дальше парковать клиентов на прямую передачу вместо пути
создания — пул перестаёт расти по требованию, оставаясь снаружи похожим
на просто загруженный. Счётчик сбрасывается только при пересоздании
объекта пула: рестартом или `RELOAD`, который пересоздаёт этот пул.

Пул со значениями `servers=3 … tracked=8` находится ровно в этом
состоянии: три бэкенда обслуживают трафик, а пул уверен, что их восемь.

**`waiters` разделяет ожидание.** Поле `wait=` в группе `clients` считает
всех клиентов между `waiting()` и получением соединения — и стоящих на
семафоре выдачи, и припаркованных внутри пула; `waiters` — только вторая
группа. Глубокая очередь `waiters` означает, что пулу не хватает
соединений; `waiters` около нуля при большом `wait=` означает, что
ограничивает `pool_size`, а каждый допущенный клиент обслуживается
быстро. Величина диагностическая, а не точная: в неё попадают и
отправители, брошенные уже отказавшимися от ожидания клиентами, — они
разбираются лишь по мере возврата соединений.
...
- `create_fallback` высокий, но `query_wait_timeout` короткий
  (< 1 секунды) → дедлайн упреждающего ожидания (`query_wait_timeout − 500 ms`
