...
Давление на пул описывает, как pg_doorman ведёт себя, когда множество
клиентов одновременно запрашивают серверное соединение, а очередь
свободных, готовых к выдаче соединений пуста.
...
**ограничитель всплесков** — жёсткий лимит на одновременные
подключения к PostgreSQL внутри одного пула. Поверх них при
необходимости подключается **координатор (coordinator)**,
общий для всех пулов ограничитель суммарного числа серверных соединений
к одной базе данных.
...
находиться в anticipation zone и иметь все слоты ограничитель всплесков занятыми
...
backend) пул уменьшает текущий размер, и вызов проваливается в путь
...
в ограничитель всплесков, создавая N новых backend-соединений для пула, которому
...
```
   ...
   |  ограничитель всплесков  |  --- slot full  --> wait, retry recycle
   +------+-------+
          |
          v
   +--------------+
...
Тот же сценарий с 200 клиентами в формате лавина одновременных подключений, но теперь
...
Фаза anticipation и ожидание у ограничитель всплесков пропускаются. pg_doorman
выполняет recycle на горячем пути, один раз пробует ограничитель всплесков, затем
либо создаёт соединение, либо возвращает ошибку wait timeout.

**Ограничение при включённом coordinator.** Режим без ожидания
пропускает только anticipation и ожидание у ограничитель всплесков внутри пути в
...
### Прямая передача при возврате (прямая передача)
...
минуя idle-очередь. Ожидающие, чей вызывающий уже отвалился по
таймауту, пропускаются: пул обнаруживает недоступного получателя и
пробует следующего в очереди.

Если зарегистрированных ожидающих нет (типичный случай при высоком
throughput, когда каждый checkout попадает в горячий путь), соединение
кладётся в idle-очередь и будит ближайшего клиента, ожидающего на
Фазе 1/2.
...
кандидатов для eviction. Ожидающие внутри того же пула получают
...
- **Отсутствие голодания.** При broadcast-notify клиент может
  проиграть гонку за wake-up многократно. При прямая передача
  соединение идёт ровно одному получателю, протухшие waiter'ы
  пропускаются. Нет лавина одновременных подключений, нет повторных проигрышей.
...
задержки. Когда checkout проверяет пригодность соединения и
обнаруживает, что оно достигло **95%** своего lifetime, фоновая задача
создаёт замену и помещает её в idle-очередь. Когда старое соединение
...
Упреждающая замена срабатывает только на пути **checkout** (при
проверке пригодности), не из фоновой задачи обслуживания.
Idle-соединения, истекающие без checkout'а, закрываются фоновой
...
2. Соседний пул вернул соединение в свою idle-очередь — слот
...
один соседний backend не закрывается. Пробуждение от idle-return не
освобождает слот напрямую, но могло вырастить излишек соседа, поэтому
повторная попытка eviction находит кандидата, которого мгновение назад
...
берётся только когда вызов уже занял слот ограничителя всплесков и готов вызвать
`connect()`.

Предыдущий порядок (координатор первый, потом gate) вызывал
**фантомные permit-ы (phantom permits)**: N вызовов захватывали по coordinator-permit и
вставали в очередь за ограничителем всплесков (cap=2). Реально создавали соединения
...
ограничителя**, ждёт координатора (eviction / возврат от соседа), и
...
```
   ...
   Phase 3: fast spin          --- HIT ---> return
       | MISS                               |
       v                                    |
   Phase 4: прямая передача     --- HIT ---> return
       | deadline                           |
       v                                    |
       | <----------------------------------+
       v
   Phase 5: ограничитель всплесков (scaling_max_parallel_creates)
              | slot acquired
              v
   +---------------------------+
...
ограничитель всплесков: фоновая задача не должна бороться с клиентским трафиком
за скудные permit-ы.
...
запроса), но клиенты видят p99 задержка выдачи соединения 100–500 ms на базе под
...
| До                              | После                              | Вердикт                                                                     |
|---|---|---|
| `reserve_acq` не растёт         | `reserve_acq` растёт               | Reserve-first подхватил — задержка выдачи соединения должен упасть; ожидаемо          |
...
| `replenish_def` | counter | Запуски фонового replenish, упёршиеся в лимит ограничитель всплесков и отложенные до следующего retain-цикла. Устойчиво ненулевые значения означают, что `min_pool_size` нельзя поддержать при текущей нагрузке. |
...
| Прогрев `min_pool_size` | Поддерживается на каждом такте event loop (без отдельной задачи replenish). | Периодический фоновый replenish (`retain_connections_time`, по умолчанию 30 s), который отступает, когда ограничитель всплесков занят. |
...
В промышленной эксплуатации важнее всего два различия:
...
- **permit координатора (coordinator permit)** — разрешение на
  ...
  backend-соединение физически уничтожается (а не когда оно
  возвращается в idle-очередь); при освобождении слот возвращается
...
- **эффективный минимум** — пол для eviction у user-пула,
  ...
  выселения соседями.
- **прямая передача (прямая передача)** — механизм доставки в Фазе 4.
  При возврате соединение отправляется напрямую старейшему
  зарегистрированному ожидающему, минуя idle-очередь. Соединение
...
- **жёсткий потолок Фазы 4** — каждый checkout выбирает случайный
  ...
  Фазы 4 anticipation, независимо от `query_wait_timeout`. Не
...
- **повышение из резерва в основной (reserve → main upgrade)** —
  периодическая бухгалтерская перестановка. Когда idle-backend
  удерживает reserve-permit, а в основном семафоре есть запас, retain
  task забирает основной permit, возвращает резервный слот и
...
- **излишек над минимумом (spare_above_min)** — текущий размер пула
  ...
  всех соединений пула (активные + idle вместе, а не только idle).
  Координатор использует это значение, чтобы выбрать жертву eviction:
  пул с самым большим излишком теряет соединение первым. Само
  соединение, чтобы его можно было выселить, всё равно должно быть
  свободным — излишек выбирает пул, а не конкретное соединение.
- **голодающий пользователь (starving)** — user-пул, у которого текущее число
...
- **пул под давлением (under pressure)** — состояние, при котором
  ...
  Retain task пропускает повышение/закрытие на таких пулах, потому
