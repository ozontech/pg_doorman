...
4. Из списка members doorman собирает список TCP-живых кандидатов в
   порядке приоритета: сначала `sync_standby`, потом `replica`, потом
   любой другой. TCP probe запускается параллельно; не отвечающие
   кандидаты выпадают из списка.
5. doorman перебирает список и выполняет `Server::startup` для каждого
   кандидата, ограниченный `fallback_connect_timeout` (по умолчанию
...
8. Последующие соединения в рамках cooldown идут к тому же
...
9. Когда cooldown истекает, doorman снова пробует локальный socket.
...
последовательный отказ того же хоста удваивает cooldown, capped на
60 секундах. После окончания окна запись удаляется, счётчик
сбрасывается на следующем отказе. Это не даёт «застрявшему»
кандидату (postgres в recovery, persistent ошибка auth, медленная
сеть) повторно пробоваться на каждый клиентский запрос и
заваливать одновременно и кандидата, и Patroni API.

Карта cooldown ограничена 256 записями; при превышении лимита
истёкшие записи удаляются перед очередным insert.
...
| `fallback_connect_timeout` | `"5s"` | TCP probe timeout, дедлайн для `Server::startup` каждого кандидата и база per-host cooldown после отказа startup. Один параметр на три роли — все три имеют одинаковую семантику «кандидат не отвечает». |
...
host), gauge для предыдущего `(host, port)` удаляется атомарно с
установкой нового. Дашборды не видят два хоста одновременно
помеченных как активные во время переключения.
