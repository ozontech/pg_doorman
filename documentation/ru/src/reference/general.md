...
имя параметра и пользователя — в warn-строке лога. Эффективный
каскад per pool виден через `SHOW STARTUP_PARAMETERS` в admin
SQL-консоли и через `/api/pools` в Web UI.
