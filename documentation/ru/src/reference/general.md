...
pg_doorman проигрывает параметры сессии на каждом новом бэкенде перед выполнением транзакции, чтобы
бэкенд видел то же состояние сессии, которое запросил клиент.

Replay питается из двух источников:

1. **Обновления PG `ParameterStatus`** (всегда отслеживаются при включении): `client_encoding`,
   `DateStyle`, `IntervalStyle`, `TimeZone`, `standard_conforming_strings`, `application_name`.
   Это GUC'и, о смене которых сам PostgreSQL сообщает клиенту по wire.

2. **Ключи клиентского `StartupMessage`** (с pg_doorman ≥ 3.10): любой параметр, который клиент
   прислал в стартовом пакете, кроме read-only / server-managed имён (`is_superuser`,
   `server_version`, `lc_collate`, `transaction_isolation`, …) и зарезервированного префикса
   `_pq_.`. Это то, что позволяет клиенту закрепить `search_path`,
   `default_transaction_isolation`, `role` и другие planner-видимые GUC'и в момент подключения
   и иметь их на любом backend, куда попадёт транзакция. Operator-managed `startup_parameters`
   всегда перебивают клиентский пакет.

**Ограничения области — прочитать прежде чем полагаться на это для корректности:**

- Replay работает **только для startup**. Клиент, который выполнит `SET search_path = …`
  (или любой другой planner GUC) *после* `AuthenticationOK`, изменит состояние backend, но
  snapshot pg_doorman для этого клиента обновлён **не будет** — PG не присылает
  `ParameterStatus` для имён вне `GUC_REPORT`, поэтому pg_doorman не имеет сигнала.
  Последующее переиспользование prepared statement на этом backend может попасть на план,
  построенный под устаревшим GUC. Клиенты, которые меняют planner GUC в runtime, должны либо
  выставлять их в `StartupMessage`, либо после изменения делать `DISCARD ALL` / переподключаться,
  либо выключить `prepared_statements` для пула.

- Ключ кэша prepared statement складывает digest *startup-pinned* planner GUC'ов
  (`PLANNER_KEYS = {search_path, default_transaction_isolation, default_transaction_read_only,
  default_text_search_config, role}`) поверх legacy-ключа `query text + parameter OID`. Другие
  planner-релевантные GUC'и, которые pg_doorman пушит по тому же каналу — `TimeZone`,
  `DateStyle`, `plan_cache_mode`, `enable_*`, JIT cost knobs, extension GUC — **не** входят в
  ключ кэша. Два клиента с одинаковым `query` + `param OIDs`, но разными значениями этих
  ключей разделят одну запись кэша и один план. Если смешивать workload, который готовит один
  и тот же query text под разными такими GUC, либо выключите `prepared_statements` для пула,
  либо закрепите GUC на уровне роли или базы.

Стоит дополнительный roundtrip за checkout, когда diff не пустой. Если нужна только видимость
