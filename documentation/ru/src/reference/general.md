...
Parse) **всегда без лимита** — этот параметр на неё не влияет. Named statements живут в кеше до
...
Меньшие значения быстрее сжимают интернер после волн отключений ценой большей
нагрузки на CPU. Значение `0` отвергается на старте.

**Только при перезапуске**: изменения подхватываются только после рестарта;
config reload не меняет частоту sweep'ов работающего процесса.
...
на следующем sweep'е, который всё ещё видит её бездействующей.

Значение `0` полностью отключает TTL: анонимные записи живут до перезапуска процесса.
Это соответствует поведению до 3.7 и нужно для legacy-инсталляций, которые опираются
на кросс-batch unnamed prepared statements; в остальных случаях оставляйте значение по умолчанию.

Перечитывается на каждом sweep'е, поэтому config reload меняет эффективный TTL без рестарта.
