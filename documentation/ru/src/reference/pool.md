...
Параметры запуска PostgreSQL уровня пула, которые записываются в
`StartupMessage` каждого бэкенда. Значения переопределяют
`general.startup_parameters` по ключу. В passthrough-пулах `auth_query`
пользовательская колонка `startup_parameters` может переопределить и этот
уровень.
...
каждым запуском бэкенда слитый каскад снова проверяется против
