...
5. **Tiebreaker — p95 времени транзакции.** Среди пулов с одинаковым излишком первым уступает тот, у кого выше p95. Высокий p95 означает, что каждая транзакция дольше держит соединение; тот же пользователь реже переиспользует одно и то же соединение, и одно вытеснение «стоит» меньше потерянных reuse-checkout-ов.
