...
PgBouncer (1.21+) и Odyssey поддерживают prepared statements в transaction mode, но только для **именованных** statement; анонимный `Parse` пробрасывается без изменений и каждый раз перепланируется. PgDoorman — тот, кто его переписывает.

В 3.7.0 этот кеш стал ограниченным и наблюдаемым. Глобальный интернер, в котором лежит текст `Parse`, разделён на NAMED (пассивный сборщик по `Arc::strong_count`) и ANON (TTL по бездействию, 60 с по умолчанию); клиентский кеш разделён на Named (без ограничения) и Anonymous (LRU с порогом `client_anonymous_prepared_cache_size`). `SHOW INTERNER`, `SHOW INTERNER N` и `RESET INTERNER` показывают содержимое и позволяют сбросить кеш; пять метрик `pg_doorman_query_interner_*` покрывают записи, байты, вытеснения по каждой половине, синтетические промахи и длительность сборки. На `Bind` к истёкшему анонимному prepared теперь возвращается SQLSTATE `26000` (как у PostgreSQL), и стандартные драйверы прозрачно отвечают повторным `Parse`.
...
- **Ограниченная память prepared-кеша.** Обе половины кеша (интернер и клиентский) разделены по типу `Parse`: поток анонимных запросов не вытеснит именованные записи, а TTL не даёт анонимной половине копить мёртвые тексты. Драйверы видят свои имена, бекенды — переименованные `DOORMAN_N`. Никаких прикладных `DEALLOCATE`, никаких `DISCARD ALL`.
