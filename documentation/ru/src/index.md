...
```admonish success title="Обновление бинаря с переносом сессий"
В отличие от обычного rolling restart, PgDoorman переносит уже открытые клиентские сессии в новый процесс. В foreground-режиме свободный клиент остаётся на том же TCP-соединении: старый процесс передаёт его файловый дескриптор через `SCM_RIGHTS` и сериализует состояние, без которого пулеру пришлось бы начинать заново.
...
Клиенты внутри транзакции остаются на старом процессе и переезжают после `COMMIT` или `ROLLBACK`. У PgBouncer (`-R`, устарел с 1.20, или rolling restart через `so_reuseport`) и Odyssey (`SIGUSR2` + `bindwith_reuseport`) старые сессии остаются в старом процессе до отключения клиентов. TLS-клиенты мигрируют только в Linux-сборке с фичей `tls-migration`; по умолчанию они дренируются.
