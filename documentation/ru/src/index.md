...
```admonish success title="Обновление бинаря без остановки"
PgDoorman обновляется по `SIGUSR2` без остановки приёма подключений. Новый процесс начинает принимать соединения, а старый передаёт ему свободные клиентские сессии без повторного TCP-подключения. Это снижает всплеск `auth`/SCRAM к PostgreSQL во время обновления.

Свободная сессия переезжает как тот же клиентский сокет через `SCM_RIGHTS`, вместе с ключами отмены запросов и состоянием кеша prepared statements. Клиенты внутри транзакции остаются на старом процессе и переезжают после завершения транзакции. Перенос TLS-состояния OpenSSL доступен только в Linux-сборке с фичей `tls-migration`; по умолчанию она выключена.

У PgBouncer (`-R`, устарел с 1.20, или rolling restart через `so_reuseport`) и Odyssey (`SIGUSR2` + `bindwith_reuseport`) новые подключения переходят на новый процесс, а старые сессии остаются на прежнем до отключения клиентов. Отличие PgDoorman — перенос свободных сессий и подготовленного состояния между процессами.
