...
В 3.7.0 этот кеш стал ограниченным и наблюдаемым. Глобальный interner, в котором лежит текст `Parse`, разделён на NAMED (пассивный GC по `Arc::strong_count`) и ANON (TTL по бездействию, 60 с по умолчанию); per-client кеш разделён на Named (без ограничения) и Anonymous (LRU с порогом `client_anonymous_prepared_cache_size`). `SHOW INTERNER`, `SHOW INTERNER N` и `RESET INTERNER` дают взгляд на содержимое и сброс; пять метрик `pg_doorman_query_interner_*` покрывают записи, байты, выселения по каждой половине, синтетические промахи и длительность GC. На `Bind` к истёкшему анонимному prepared теперь возвращается SQLSTATE `26000` (как у PostgreSQL), и стандартные драйверы прозрачно отвечают повторным `Parse`.
...
- **Ограниченная память prepared-кеша.** Обе половины кеша (interner и per-client) разделены по типу `Parse`: поток анонимных запросов не вытеснит named-записи, а TTL не даёт анонимной половине копить мёртвые тексты. Драйверы видят свои имена, бекенды — переименованные `DOORMAN_N`. Никаких прикладных `DEALLOCATE`, никаких `DISCARD ALL`.
