...
проиграна (соседний ожидающий забрал вернувшееся соединение первым),
...
30 секунд) и делает бухгалтерский swap: для каждого пула без давления
он обходит idle vec и для каждого backend-а, удерживающего
reserve-permit, пытается забрать permit из main-семафора. При успехе
...
короткие всплески, а retain task каждые `retain_connections_time`
(по умолчанию 30 секунд) апгрейдит idle reserve-permit-ы обратно в
main. Устойчивое ненулевое `reserve_in_use` на протяжении 15 минут
означает одно из двух: либо main постоянно упёрт в
`max_db_connections` (и апгрейдам некуда забрать main-слот), либо
backend-ы с reserve-permit-ами никогда не простаивают достаточно,
чтобы retain task их коснулся. В любом случае ёмкость базы занижена
...
пуле без давления происходит swap permit-а на main, если в
