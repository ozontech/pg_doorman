...
Берём меньшее из двух значений, но не менее 1 ms, чтобы wait успел
зарегистрироваться. Если возврат происходит во время ожидания,
просыпается ровно **одна** задача, а не все сразу. Если ожидание
истекает без возврата, переходим к фазе 5.
...
| `scaling_max_anticipation_wait_ms` | `100` (ms) | `general` | Верхняя граница ожидания на `Notify` возврата idle-соединения перед переходом к backend-connect. Ограничивается половиной оставшегося `query_wait_timeout` клиента. |
