...
Третья зона накладывается поверх обеих: при любом размере пул��, если
число соединений, создаваемых прямо сейчас, ��остигает
...
1. Был ос��обождён permit координатора соседнего пула — серверное
...
не было, освобождает permit соседа, и следую��ая неблокирующая проверка
...
- **эффективный минимум** — пол для eviction у user-пула,
  ...
  выселения соседями.
- **прямая передача (direct handoff)** — механизм доставки в Фазе 4.
  При возврате соединение отправляется напрямую старейшему
  зарегистрированному ��жидающему, минуя idle-очередь. Соединение
...
- **жёсткий потолок Фазы 4** — каждый checkout выбирает случайный
  ��отолок в диапазоне 300–500 ms. Верхняя граница времени ожидания
