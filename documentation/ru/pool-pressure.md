...
Дедлайн равен `query_wait_timeout` клиента минус 500 ms запаса на
...
В стоковой конфигурации pg_doorman параметр не влияет ни на что. При
заданном `query_wait_timeout` цикл anticipation работает на полном
остатке клиентского бюджета (минус 500 ms резерв на создание
соединения), и `scaling_max_anticipation_wait_ms` в расчёт не идёт.
Менять его значение бессмысленно: поведение не изменится.

Параметр применяется только когда вызывающий передаёт
`Timeouts { wait: None, .. }`. В стоке pg_doorman такого пути нет:
`query_wait_timeout` всегда выставлен в секции `general` и
прокидывается в `Timeouts.wait`. Считайте этот параметр предохранителем
для прямых API-потребителей, не рычагом для контроля tail latency.
