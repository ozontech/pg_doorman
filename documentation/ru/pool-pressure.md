...
ждём notify либо остаток дедлайна, пробуем `try_recycle_one` ещё раз
после пробуждения. Если гонка проиграна — соседний ожидающий забрал
вернувшееся соединение первым — итерация повторяется и ждёт следующего
возврата. Цикл завершается по успешному recycle, по пробуждению от
sleep (notify так и не пришёл) либо по достижении дедлайна.

Дедлайн — это `query_wait_timeout` клиента минус 500 ms запас на
create path, отсчёт от `start`, захваченного в начале `timeout_get`.
Фаза 1/2 ожидания семафора тратит из того же бюджета, поэтому
anticipation не может увести клиента за его `query_wait_timeout`.
Если `query_wait_timeout` не установлен, цикл берёт
`scaling_max_anticipation_wait_ms` (по умолчанию 100 ms) как общий
бюджет. Если раньше вы понижали `scaling_max_anticipation_wait_ms`
ради контроля tail latency — этот тюнинг больше не работает: при
заданном `query_wait_timeout` параметр не применяется. Цикл тратит
весь остаток клиентского бюджета на восстановление после проигранных
гонок вместо того, чтобы сразу платить за backend `connect()`.
...
| `scaling_max_anticipation_wait_ms` | `100` (ms) | `general` | Fallback-бюджет цикла anticipation, если `query_wait_timeout` не задан. При заданном `query_wait_timeout` (типичный случай) цикл берёт оставшийся бюджет клиента минус 500 ms резерв на create path, и этот параметр не применяется. Поднимать имеет смысл только для долгоживущих клиентских сессий без wait-дедлайна. |
...
### `scaling_max_anticipation_wait_ms`: когда этот параметр вообще влияет

В большинстве конфигураций — никогда. При заданном `query_wait_timeout`
цикл anticipation работает на полном остатке клиентского бюджета
(минус 500 ms резерв на создание соединения), и
`scaling_max_anticipation_wait_ms` в расчёт не идёт. Менять его значение
бессмысленно: поведение не изменится.

Параметр применяется только когда `query_wait_timeout` не задан и
циклу нужен какой-то дедлайн. Такие конфигурации редки; в production
`query_wait_timeout` обычно выставлен. Если у вас есть долгоживущие
клиентские сессии без wait-дедлайна и вы хотите дать им больше
времени на перехват возврата вместо `connect()` — поднимайте этот
параметр. В остальных случаях оставляйте значение по умолчанию.

Если раньше вы понижали `scaling_max_anticipation_wait_ms` ради
контроля tail latency, уберите этот тюнинг: он не работает. Границу
tail latency anticipation теперь задаёт `query_wait_timeout` клиента
напрямую — цикл не может взять больше, чем клиент готов ждать.
...
| `antic_timeout` | counter | Итерации, в которых `sleep` завершился раньше `notified`, либо дедлайн истёк в начале итерации. Такой инкремент завершает цикл: клиент проваливается в create path. При заданном `query_wait_timeout` высокий `antic_timeout` значит, что клиенты упираются в клиентский бюджет, а не в `scaling_max_anticipation_wait_ms`. |
...
**Anticipation create fallback rate (warn).** Цикл anticipation
стабильно не успевает поймать возврат в пределах клиентского
`query_wait_timeout`, и пул платит свежими `connect()`. Здоровое
соотношение: `create_fallback` близок к нулю при ненулевом
`creates_started`. Если фоллбек идёт устойчивым потоком, либо
`query_wait_timeout` короче типичной длительности запроса, либо
пулу не хватает размера для текущей нагрузки.
**Действие:** сравните `query_wait_timeout` с p95 длительности
запроса. Если `query_wait_timeout` короче — поднимайте его.
Иначе поднимайте `pool_size` (или `max_db_connections`, если стоит
координатор).
