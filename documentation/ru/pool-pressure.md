...
Pool size:  0 ----------- 8 ---------------------------- 40
            ...
            |  and 4.     |   (deadline = wait_timeout   |
            |  Go straight|    - 500ms, shared with      |
            |  to phase 5 |    Phase 1/2 semaphore wait) |
...
**Фаза 4 — цикл anticipation.** Если spin не поймал возврат, задача
входит в цикл ожидания на `Notify`, который будит любой
`return_object()`. Каждая итерация: регистрируем `notified`, пробуем
`try_recycle_one` до ожидания (чтобы поймать буферизованный notify),
ждём notify либо per-iteration sleep с верхней границей 100 ms,
пробуем `try_recycle_one` ещё раз после пробуждения. Если гонка
проиграна (параллельная задача забрала вернувшееся соединение первой),
итерация инкрементирует счётчик проигранных гонок и регистрирует новый
`notified` для следующего возврата. Цикл завершается по трём условиям:
успешный recycle, истёкший дедлайн в начале итерации, или счётчик
проигранных гонок достиг `MAX_RACE_LOSSES` (20) — это означает, что
задача просыпалась много раз и каждый раз теряла пост-пробуждение
recycle-гонку, поэтому anticipation сдаётся и проваливается в create
path.

Дедлайн равен `min(query_wait_timeout - 500 ms, PHASE_4_HARD_CAP)`,
где `PHASE_4_HARD_CAP = 500 ms`. Hard cap ограничивает tail latency
при патологическом порядке пробуждений, когда задача будит каждый
notify, но теряет каждую пост-пробуждение гонку — без него большой
`query_wait_timeout` позволил бы одной задаче просидеть в anticipation
десятки секунд. Отсчёт идёт от `start`, захваченного в начале
`timeout_get`; Фаза 1/2 ожидания семафора тратит из того же бюджета,
поэтому суммарное ожидание не может увести клиента за его
`query_wait_timeout`.

Если возврат приходит во время ожидания, просыпается ровно **одна**
задача, а не все сразу. Если дедлайн цикла истёк без успешного
...
### Conditional notify при возврате

Когда соединение возвращается в idle-очередь, `return_object` сигналит
двум каналам: `semaphore.add_permits(1)` для Phase 1/2 ожидающих, и
`idle_returned.notify_one()` для Phase 4 anticipation.

При высокой конкурентности (10k+ клиентов, пул полностью загружен)
idle-очередь оборачивается быстро и никто не сидит в Phase 4 — каждый
checkout попадает в hot path. Вызов `notify_one()` на каждый return
в этом случае будит задачу, которая соревнуется с semaphore-waiter'ом
за то же соединение, проигрывает и тратит ~2-5 us CPU впустую.

Чтобы этого избежать, `return_object` проверяет атомарный счётчик
`idle_returned_listeners` перед вызовом `notify_one()`. Phase 4 и
burst gate инкрементируют счётчик при регистрации `Notified` future
и декрементируют при выходе из ожидания. Когда счётчик равен нулю
(обычный случай при высоком throughput), `notify_one()` пропускается —
один atomic load (~3 ns) вместо notify-цикла (~104 ns) плюс
бесполезного пробуждения задачи.

Страховка: если listener зарегистрируется между load и пропущенным
notify, `SLEEP_CAP` Phase 4 (100 ms) и `BURST_BACKOFF` burst gate
(5 ms) гарантируют прогресс на следующей итерации.
...
3. **Eviction (выселение).** Fallback на случай, когда резерв
   ...
   пользователя, чтобы освободить main-слот. Выселяемый пул теряет
   ...
   **effective minimum** (эффективным минимумом), и только соединения
   старше `min_connection_lifetime` (по умолчанию 30 000 ms) попадают
   в список кандидатов. 30-секундный порог намеренно высокий: он
...
арбитр отказал. Обходим все *остальные* user-пулы той же базы, ищем
тот, у кого наибольший излишек над его **эффективным минимумом**, и
закрываем одно из его idle-соединений старше `min_connection_lifetime`.
...
| `scaling_fast_retries` | `10` | `general`, per-pool | Число `yield_now`-spin retry в фазе anticipation перед переходом к циклу ожидания на `Notify` (пробуждение по событию возврата, без активного опроса). |
...
| `antic_notify` | counter | Итерации цикла anticipation, проснувшиеся по реальному `idle_returned`. Цикл может зафиксировать несколько таких пробуждений на одно клиентское ожидание: после каждой проигранной гонки он возвращается ждать следующий возврат. Высокий `antic_notify` при низком `create_fallback` — хороший признак: цикл крутится, ловит возвраты, клиенты не платят за `connect()`. |
| `antic_timeout` | counter | Итерации, в которых `sleep` завершился раньше `notified`, либо дедлайн истёк в начале итерации. Такой инкремент завершает цикл: клиент проваливается в create path. Высокий `antic_timeout` означает, что клиенты упираются в `query_wait_timeout`, не успев перехватить возврат. |
| `create_fallback` | counter | Число выходов из цикла anticipation без успешного recycle: дедлайн исчерпан или бюджет был нулевой. Именно эти ожидания превращаются в новый `connect()`. Проигранные гонки здесь не учитываются: пока дедлайн не достигнут, цикл повторяет ожидание. Стабильно ненулевой `create_fallback` значит, что клиентского бюджета не хватает на перехват возвратов: пул либо мал, либо запросы длиннее `query_wait_timeout`. |
...
**Причина.** Цикл anticipation держит клиентов в ожидании возврата до
`query_wait_timeout`, но возвраты приходят медленнее, чем клиент
готов ждать. Ожидающие либо получают соединение из вернувшихся, либо
проваливаются в create path под конец своего бюджета. Проверьте
`create_fallback` в `SHOW POOL_SCALING`: если он ненулевой и растёт,
клиенты уходят по таймауту.
...
Если `create_fallback` нулевой, пул не узкое место. Задержка приходит
откуда-то ещё: PostgreSQL, сеть, клиентская сторона. Проверьте
`SHOW STATS avg_wait_time`.
...
разгоняется, а `create_fallback` перестаёт расти: цикл anticipation
...
- **`bounded burst gate`** — per-pool ограничитель, пропускающий не
  ...
  `connect()` к бэкенду. Задачи сверх лимита ждут на `Notify`, пока
  слот не освободится.
...
- **эффективный минимум** — пол для eviction у user-пула,
  ...
  выселения соседями.
- **`MAX_RACE_LOSSES`** — compile-time константа (20). Цикл
  anticipation в Фазе 4 сдаётся после стольких подряд неудачных
  пост-пробуждение recycle-попыток. Не настраивается.
- **Фаза R (reserve-first)** — короткое замыкание координатора,
  ...
  клиента в Фазе C.
- **`PHASE_4_HARD_CAP`** — compile-time константа (500 ms). Верхняя
  граница wall-time Фазы 4 anticipation, независимо от
  `query_wait_timeout`. Защищает tail latency при патологическом
  порядке пробуждений. Не настраивается.
