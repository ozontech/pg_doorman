...
где `PHASE_4_HARD_CAP = 500 ms`. Фаза 1/2 ожидания семафора тратит
из того же бюджета, поэтому суммарное ожидание не может увести
клиента за его `query_wait_timeout`. Если дедлайн истёк без получения
соединения, переходим к фазе 5.
...
- **`PHASE_4_HARD_CAP`** — compile-time константа (500 ms). Верхняя
  ...
  `query_wait_timeout`. Не настраивается.
